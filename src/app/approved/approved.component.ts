...
export class ApprovedComponent implements OnInit {
  ...
  rowClick(gatePass:GatePass){
    this._router.navigate(['/final',gatePass.slNo])
  }
  getApprovedMaterialCount(gatePassItem:Items[]): number{
    return gatePassItem.filter(x=>x.materialsApproved == 1).length;
  }
  getNotApprovedMaterialCount(gatePassItem:Items[]): number{
    return gatePassItem.filter(x=>x.materialsNotApproved == 1).length;
  }
}
